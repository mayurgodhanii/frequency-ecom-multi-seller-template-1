...
//         return acc + (cur.sum || cur.total_price)
//     }, 0 );
// }
...
export const cartPriceTotal = cartItems => {
    return cartItems.reduce((acc, item) => {
      ...
      const sum = isUSD
      ? item.total_usd_price || item.selected_variant_usd_price * item.quantity
      : item.total_price || item.selected_variant_price * item.quantity;
...
export const cartQtyTotal = cartItems => {
    return cartItems.reduce( ( acc, cur ) => {
        return acc + parseInt( cur.quantity || cur.qty, 10);
    }, 0 );
}
...
//  * @return { Float } totalPrice
//  */
// export const cartPriceTotal = cartList => {
//     return cartList.reduce((acc, cur) => {
//         return acc + cur.total_price;  // Use total_price instead of sum
//     }, 0);
// };
...
// export const cartQtyTotal = cartItems => {
//     return cartItems.reduce((acc, cur) => {
//         // Use cur.quantity if it exists, otherwise use cur.qty
//         return acc + parseInt(cur.quantity || cur.qty, 10);
//     }, 0);
// };
