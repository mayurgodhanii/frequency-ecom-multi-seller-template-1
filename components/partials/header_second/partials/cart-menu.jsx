...
function CartMenu(props) {
  ...
  const getItemPrice = (item) => {
    ...
    const price = isINR ? item.selected_variant_price : item.selected_variant_usd_price;
    const salePrice = isINR ? item.sale_price : item.usd_sale_price;
    return item.is_sale_product && salePrice ? parseFloat(salePrice) : parseFloat(price);
  };

  const getTotalPrice = () => {
    return cartList.reduce((acc, item) => acc + item.quantity * getItemPrice(item), 0);
  };
                  ...
                  <div className="product justify-content-between" key={index}>
                    ...
                    <figure className="product-image-container ml-2">
                      ...
                      >
                        <img
                          src={
                            item?.productDetails?.image?.[0] || item?.image?.[0]
                          }
                          alt="product"
                        />
                      </ALink>
