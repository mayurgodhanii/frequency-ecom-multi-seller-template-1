...
function StoresPage() {
    ...
    const stores = storesData?.success ? storesData.data.filter(store => 
        store && 
        store.id && 
        store.store_name && 
        (store.slug || store.id)
    ...
    return (
        <main className="main stores-page">
            <Helmet>
                <meta name="keywords" content="stores, shops, vendors" />
                <meta name="description" content="Browse all available stores" />
                <title>{`Stores | ${spaceName}` || "Stores"}</title>
            </Helmet>

            <PageHeader title="Stores" subTitle="Shop" />
                        ...
                        <div className="toolbox mb-3">
                            <div className="toolbox-left">
                                <div className="toolbox-info">
                                    Showing <span>{stores.length}</span> Stores
                                </div>
                            </div>
                        </div>

                        <div className="stores-grid-list">
